...
function main() {
  ...
  events.on(getAppContext(), "click", () => {
    ...
    if (event.target.classList.contains("delete-game__btn")) {
      var gameId = event.target.parentElement.querySelector(".delete-game__btn")
        .id;
      console.log(gameId);
  ...
  events.on(getAppContext(), "click", () => {
    ...
    if (event.target.classList.contains("add-game__submit")) {
      ...
      const yearReleased = document.querySelector(".add-game__yearreleased")
        .value;
      const imagePath = document.querySelector(".add-game__image-path").value;
      const screenShot = document.querySelector(".add-game__title-screen")
        .value;
      const gameConsole = document.querySelector(".add-console__forgames")
        .value;
  ...
  events.on(getAppContext(), "click", () => {
    ...
    if (event.target.classList.contains("add-console__submit")) {
      const consoleName = document.querySelector(".add-console__full-name")
        .value;
      const shortName = document.querySelector(".add-console__short-name")
        .value;
      const imagePath = document.querySelector(".add-console__image-path")
        .value;
      const companyName = document.querySelector(".add-game_manufacture").value;

      api.postRequest(
        "http://localhost:8080/consoles/add",
        {
  ...
  events.on(getAppContext(), "click", () => {
    ...
    if (event.target.classList.contains("add-company__submit")) {
      ...
      const imagePath = document.querySelector(".add-company__image-path")
        .value;
